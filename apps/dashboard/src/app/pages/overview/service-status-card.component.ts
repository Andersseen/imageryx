...
const TONE_TO_BADGE_VARIANT: Record<
  ReturnType<typeof describeServiceHealth>["tone"],
  ...
  template: `
    ...
    >
      <div class="flex items-center justify-between gap-2">
        ...
        <volt-badge [variant]="badgeVariant()">{{
          description().label
        }}</volt-badge>
      </div>
      ...
      @switch (state().status) {
        ...
        @case ("error") {
          <p class="text-sm text-destructive">
            {{ errorMessage() }} — is it running on {{ url() }}?
          </p>
        }
      }
    </div>
...
export class ServiceStatusCard {
  ...
  protected readonly description = computed(() =>
    describeServiceHealth(this.state()),
  );
  protected readonly badgeVariant = computed(
    () => TONE_TO_BADGE_VARIANT[this.description().tone],
  );
