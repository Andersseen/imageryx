 ...
 */
export const errorHandler: ErrorHandler<{ Bindings: Env; Variables: RequestIdVariables }> = (err, c) => {
  const requestId = c.get('requestId');
  console.error(JSON.stringify({ requestId, error: err.message, timestamp: new Date().toISOString() }));
...
export const notFoundHandler: NotFoundHandler<{ Bindings: Env; Variables: RequestIdVariables }> = (c) =>
