...
const SUPPORTED_SORT_FIELDS = new Set<string>(["createdAt", "updatedAt", "name", "sizeBytes"]);
const SUPPORTED_DELETED_STATES = new Set<string>(["active", "deleted", "all"]);

function parseAssetFilter(c: { req: { query: (key: string) => string | undefined } }, projectId: string): AssetFilter {
  ...
  const deleted = SUPPORTED_DELETED_STATES.has(deletedRaw) ? (deletedRaw as DeletedState) : "active";
  ...
  return {
    projectId,
    folderId: folderIdRaw === undefined ? undefined : folderIdRaw === "" ? null : folderIdRaw,
    tag: c.req.query("tag") || undefined,
    mimeType:
      mimeTypeRaw && SUPPORTED_MIME_TYPES.has(mimeTypeRaw)
        ? (mimeTypeRaw as SupportedImageMimeType)
        : undefined,
    extension: (c.req.query("extension") as SupportedImageExtension | undefined) || undefined,
    visibility:
      visibilityRaw && SUPPORTED_VISIBILITIES.has(visibilityRaw)
        ? (visibilityRaw as "public" | "private")
        : undefined,
    processingStatus:
      processingStatusRaw && SUPPORTED_PROCESSING_STATUSES.has(processingStatusRaw)
...
assetsRoute.post("/upload", async (c) => {
  ...
  if (!(file instanceof File)) {
    throw new ValidationHttpError('A "file" field containing the image is required.');
  }
  ...
  const result = await uploadAsset(
    { db: c.env.DB, storage: getStorageProvider(c.env), maxUploadSizeBytes },
    {
      projectId,
      folderId,
      file: { bytes, filename: file.name, mimeType: file.type || "application/octet-stream" },
      name,
      tags,
      visibility,
  ...
  const dispatch = await dispatchProcessingJob(c.env, c.executionCtx.waitUntil.bind(c.executionCtx), result.processingJobId);
...
assetsRoute.get("/", async (c) => {
  ...
  const [items, total] = await Promise.all([assets.list(filter), assets.count(filter)]);

  const ids = items.map((asset) => asset.id);
  const [tagsByAsset, readyVariantsByAsset, folders] = await Promise.all([
    new TagRepository(c.env.DB).listForAssets(ids),
    new VariantRepository(c.env.DB).countReadyByAssetIds(ids),
    new FolderRepository(c.env.DB).listByIds(
      [...new Set(items.map((asset) => asset.folderId).filter((id): id is string => id !== null))],
    ),
  ]);
  ...
  const enriched = items.map((asset) => ({
    ...
    folder: asset.folderId
      ? (() => {
          ...
          return folder ? { id: folder.id, name: folder.name, path: folder.path } : null;
        })()
      : null,
  }));

  return c.json(buildPaginatedResponse(enriched, filter.page, filter.pageSize, total));
});
...
assetsRoute.get("/:assetId", async (c) => {
  ...
  const [project, folder, tags, presets, variants, jobs, activity, duplicates] = await Promise.all([
    new ProjectRepository(c.env.DB).findById(asset.projectId),
    asset.folderId ? new FolderRepository(c.env.DB).findById(asset.folderId) : Promise.resolve(null),
    ...
    new ProcessingJobRepository(c.env.DB).list({ projectId: asset.projectId, assetId: asset.id }),
    new AssetActivityRepository(c.env.DB).listByAsset(asset.id),
    new AssetRepository(c.env.DB).listByChecksum(asset.projectId, asset.checksum),
  ]);
  ...
  return c.json({
    ...
    presets: presets.map((preset) => ({ id: preset.id, name: preset.name, slug: preset.slug })),
    variants,
    processingJobs: jobs,
    activity,
...
assetsRoute.patch("/:assetId", async (c) => {
  ...
  if (existing.deletedAt) throw new ConflictError("asset_deleted", "This asset has been deleted.");
...
assetsRoute.post("/:assetId/move", async (c) => {
  const assetId = param(c, "assetId");
  const body = moveAssetInputSchema.parse({ ...(await c.req.json()), id: assetId });
  ...
  const updated = await assets.update(assetId, { folderId: body.folderId, path: nextPath });
...
assetsRoute.put("/:assetId/tags", async (c) => {
  ...
  const tagIds = await Promise.all(
    body.tags.map(async (name) => (await tagsRepo.findOrCreate(existing.projectId, name)).id),
  );
...
assetsRoute.get("/:assetId/activity", async (c) => {
  ...
  const activity = await new AssetActivityRepository(c.env.DB).listByAsset(assetId);
...
assetsRoute.get("/:assetId/delivery", async (c) => {
  ...
  const project = await new ProjectRepository(c.env.DB).findById(asset.projectId);
  if (!project) throw new NotFoundError("project");

  const presets = await new PresetRepository(c.env.DB).listByProject(asset.projectId);
  ...
  return c.json({
    ...
    presets: presets.map((preset) => ({
      ...
      url: buildDeliveryUrl(c.env.DELIVERY_URL, project.slug, asset.path, preset.slug),
    })),
  });
});
...
assetsRoute.post("/:assetId/download-url", async (c) => {
  const assetId = param(c, "assetId");
  const body = createDownloadUrlInputSchema.parse({ ...(await c.req.json().catch(() => ({}))), id: assetId });

  const asset = await new AssetRepository(c.env.DB).findById(assetId);
  if (!asset) throw new NotFoundError("asset");
  if (asset.deletedAt) throw new ConflictError("asset_deleted", "This asset has been deleted.");
  ...
  if (body.variant === "original") {
    ...
    const variant = await new VariantRepository(c.env.DB).findById(body.variant);
    if (!variant || variant.assetId !== assetId) throw new NotFoundError("variant");
    if (variant.status !== "ready") {
      throw new ConflictError("variant_not_ready", "This variant is not ready for download yet.");
    }
  }

  const expiresAt = Math.floor(Date.now() / 1000) + body.expiresIn;
  const token = await createSignedToken(
    { assetId, variant: body.variant, exp: expiresAt, nonce: crypto.randomUUID() },
...
assetsRoute.post("/:assetId/variants", async (c) => {
  const assetId = param(c, "assetId");
  const body = createVariantInputSchema.parse({ ...(await c.req.json()), assetId });
  ...
  if (outcome.status === "created") {
    ...
    return c.json(
      { variant: outcome.variant, processingJobId: outcome.processingJobId, status: "created" },
      202,
    );
  }

  if (outcome.status === "ready") {
    return c.json({ variant: outcome.variant, processingJobId: null, status: "ready" }, 200);
  }

  return c.json(
    { variant: outcome.variant, processingJobId: outcome.processingJobId, status: outcome.status },
    202,
  );
});
...
assetsRoute.delete("/:assetId", async (c) => {
  ...
  const existing = await assets.findById(assetId);
  if (!existing) throw new NotFoundError("asset");
  if (existing.deletedAt) throw new ConflictError("asset_already_deleted", "This asset is already deleted.");
...
assetsRoute.post("/:assetId/restore", async (c) => {
  ...
  const existing = await assets.findById(assetId);
  if (!existing) throw new NotFoundError("asset");
  if (!existing.deletedAt) throw new ConflictError("asset_not_deleted", "This asset is not deleted.");

  const project = await new ProjectRepository(c.env.DB).findById(existing.projectId);
  if (!project) {
    throw new ConflictError("project_missing", "The asset's project no longer exists.");
  }
  if (existing.folderId) {
    const folder = await new FolderRepository(c.env.DB).findById(existing.folderId);
    if (!folder) {
  ...
  const conflict = await assets.findByPublicPath(existing.projectId, existing.path);
  if (conflict && conflict.id !== existing.id) {
    throw new ConflictError(
